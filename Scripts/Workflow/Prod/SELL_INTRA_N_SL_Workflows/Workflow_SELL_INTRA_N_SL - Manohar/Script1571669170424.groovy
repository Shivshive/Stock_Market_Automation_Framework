...
WebUI.callTestCase(findTestCase('Logins/Login_Manohar'), [('userID') : 'M137505', ('password') : 'HHBM123hhbm'], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 1; i < (totalrows + 1); i++) {
    ...
    if (this.OrderType_Sell_Buy.equalsIgnoreCase('sell')) {
        WebUI.callTestCase(findTestCase('Utility/Prod/Sell_Limit_Intra_Normal_Stoploss'), [('OrderType_Sell_Buy') : OrderType_Sell_Buy
                , ('OrderType_Normal_Stoploss') : TriggerPrice, ('TriggerPrice') : TriggerPrice, ('Symbol') : Symbol, ('Quantity') : Quantity
                , ('Price') : Price, ('Disc_Qty') : Disc_Qty, ('PriceType_L_SL') : PriceType_L_SL], FailureHandling.STOP_ON_FAILURE)
    } else {
